...
const CreateStudent = ({ addStudent, myparent, errors }) => {
  ...
  return (
    <div className="dashboard absolute bg-[#f5f9fc] top-[75px] left-[16%] p-[10px]">
      ...
      {errors.non_field_errors && errors.non_field_errors.map((err, index) => (
    <p key={index} className="error">{err}</p>
        ...
        <div className="items-center justify-center w-[250px] h-[210px]">
          <label htmlFor="profile_img" className="flex flex-col items-center mb-[30px] justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50">
            ...
            {/* <input  type="file" aria-label="Choose file to upload" value={profile_image} disabled='' nChange={(e) => setProfile_image(e.target.files[0])}  className="bg-white flex-grow font-medium px-3 py-2 text-ellipsis dark:bg-gray-900 text-gray-300 dark:text-gray-300"/> */}
           
            <input type="file" aria-label="Choose file to upload"  id="profile_image" value={profile_image} nChange={(e) => setProfile_image(e.target.files[0])} className="w-[107px] flex-grow font-medium px-3 py-2 text-ellipsis dark:bg-gray-900 text-gray-300 dark:text-gray-300"/>
